...
/*
  ...
  const locations получает динамически дробленный массив данных из геттера getOpenPlaces
...
const locations = ref([...store.getOpenPlaces])
const pageSize = ref(12)
const currentPage = ref(1)


/*
  В вотчере мы следим за изменением нынешней страницы.
  Параметр newPage передаваемый в функцию явялется измененным состоянием
  функции currentPage.
 */
watch(currentPage, (newPage) => {
  /*
    ...
    пушим в него новое значение:
      ...
      был путь: http://localhost:3000/PlacesInside

      стал путь: http://localhost:3000/PlacesInside?page=2
...
onMounted(async () => {
  ...
  if(!store.getCategoriesDataAll.length) {
    await store.fetchData().then(() => {
      locations.value = store.getOpenPlaces
    });
  }
...
<template>
  <v-container class="my-5 py-5">
    ...
    >
      <VRow>
        <VCardTitle>Title {{currentPage}}</VCardTitle>
      </VRow>
      <!--
         Проверяем всю VRow на наличие даны внутри переменной locations.
      ...
      <VRow
        ...
        <!--
          ...
          place становится каждым элементов внутри locations[currentPage - 1]
          index иногда нужен для различия элементов по нему

          locations является массивом массивов = [[12][12][12]]
          то есть мы обращаемся уже к каждому конкретному массиву
          locations["динамический номер страницы" - 1]
